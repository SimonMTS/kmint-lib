...
node_list a_star::find_path(map_node& start, const map_node& end,
                       const map_graph& graph, const heuristic& heuristic) {
  ...
  auto cmp = [](const map_node& lhs, const map_node& rhs) {
    return true;
  };

  std::vector<std::reference_wrapper<map_node>> asd;
  std::priority_queue<std::reference_wrapper<map_node>,
                      std::vector<std::reference_wrapper<map_node>>,
                      decltype(cmp)>
      open_list(cmp);
  open_list.push(tmp); // <-- ??
  ...
  //  open_list.pop();
    ...
    //for (auto& edge : shortest) {
      ...
      //      weight_up_to_this_node + estimated_total_weight_to_end; // aka F

      //  if (estimated_total_weight_to_end < weights.at(edge.to().node_id()).first) {
